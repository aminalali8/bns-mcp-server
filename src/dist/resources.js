 ...
 */
function setupStaticResources(server) {
    // Overview of Bunnyshell
    server.resource("overview", "bunnyshell://overview", async (uri) => ({
        contents: [{
                uri: uri.href,
                text: `
...
The Bunnyshell CLI (bns) provides command-line access to all Bunnyshell functionality.
    ...
    // Commands reference
    server.resource("commands", "bunnyshell://commands", async (uri) => ({
        contents: [{
                uri: uri.href,
                text: `
...
- ssh - SSH into running containers
    ...
    // Common workflows
    server.resource("workflows", "bunnyshell://workflows", async (uri) => ({
        contents: [{
                uri: uri.href,
                text: `
 ...
 */
function setupResourceTemplates(server) {
    // Environment details
    server.resource("environment-details", new ResourceTemplate("bunnyshell://environments/{environmentId}", {
        list: async () => {
            try {
...
- https://${environmentId}-api.demo-project.bunnyshell.com
    ...
    // Project information
    server.resource("project-details", new ResourceTemplate("bunnyshell://projects/{projectId}", {
        list: async () => {
            try {
...
- 3 secrets
    ...
    // Component details
    server.resource("component-details", new ResourceTemplate("bunnyshell://components/{componentId}", {
        list: async () => {
            try {
...
- Storage: 1Gi
    ...
    server.resource("template-details", new ResourceTemplate("bunnyshell://templates/{templateId}", {
        list: async () => {
            try {
